...
| Date | Phase | Note |
| ---- | ----- | ---- |
| 2026-08-13 | 7 — sales & storefront | Exit green: `bun run ci` end-to-end (typecheck + verify:db + verify:rbac + test:rbac 12/12 + verify:immutability + verify:hygiene + verify:deps + verify:stock + verify:audit 14 routes + smoke:catalog + smoke:media + smoke:inventory + smoke:purchasing + smoke:sales + smoke:storefront + test:core 17/17 + test:stock 7/7 + test:sales 1/1); `bunx drizzle-kit generate` → zero schema drift (order/invoice/payment tables landed in phase 1). Delivered: `services/sales.ts` (header-only orders — lines live on the linked draft invoice, never the order row; `createOrder`/`updateOrder`/`confirmOrder` (creates the **empty** draft invoice)/`cancelOrder` (draft\|pending → cancelled, voids linked draft); `resolveSaleLines` — regular `{variantId, quantity}` re-priced server-side from `variants.sellingPricePaise`/`products.gstRatePct` (client price on a regular line → 400, duplicate regular variantId → 400), custom `{isCustomItem:true, name, quantity, unitPricePaise}` kept at zero tax; `createInvoiceDraft`/`updateInvoice` — full-replace, versioned, `409 stale_version`, draft-only, totals always derived; `issueInvoiceCore` — the one money/stock core behind staff issue/POS/COD (actorType `"staff"` default \| `"customer"`), re-derives money from `invoice_items` snapshots, FIFO via `batchHoldings`/`allocateBatches`, writes `sale` movements, snapshots totals, marks `issued`, `invoice.issued` event when `orderId` set; 409 reasons: `insufficient_stock`, `already_issued`, non-draft `invalid_transition` (void), re-issue → 409 zero rows; `posCheckout` — one-step create+confirm+issue in one tx; `voidInvoice` draft-only; `listOrders`/`getOrder`/`listInvoices`/`getInvoice` — detail with events timeline + computed `outstandingPaise`), `services/cart.ts` (`listCart`/`upsertCartLine`/`deleteCartLine`, wishlist, `listAddresses`/`getAddress`/`createAddress` — own rows only), `services/checkout.ts` (`resolveStorefrontOutlet` — `settings.defaultOutletId` else first active (createdAt, id), none → 500 `no outlet configured`; `storefrontCheckout` — re-reads cart, re-prices from current variant, in-tx stock gates; `cod` → pending → confirmed + issue + cart cleared in one tx; `gateway` → per-line pre-gate (`sumStock ≥ qty`) but allocates nothing: order stays pending, invoice stays draft, `pending` payments row inserted (`mode=gateway`, `gatewayPaymentId` = checkout reference returned as `checkoutReference`), cart kept until phase-8 webhook confirm; pending row never updated — payments insert-only; `listCustomerOrders`/`getCustomerOrder`/`cancelCustomerOrder` — own-only, pending-only, voids linked draft), `services/returns.ts` (`createSalesReturn` — draft `RT-` doc, own confirmed order (else 409 `invalid_transition`), invoice must be issued (404), lines `{originalItemId, quantity}` referencing the invoice's own line (404), qty ≤ original (400), custom lines rejected (400), money copied from the original line), `routes/sales.ts` (api.md §6, guards R(canManageSales)/S exactly per spec), `routes/storefront.ts` (api.md §9, public catalog/products — only products with ≥1 `isCustomerVisible` active variant, stock as `isInStock` only; rate-limit 5/min via new `lib/rate-limit.ts` keyed by customer userId), mounted after payments in `app.ts`; `scripts/smoke-sales.ts` + `scripts/smoke-storefront.ts` (both PASS, stable ×3; deterministic FIFO via pre-created batches SB-A/SB-B with expiryDates; POS money 5876 (4800+576), draft quote zero stock, double-issue 409 `already_issued` zero extra movements, client-price injection → 400, duplicate lines → 400, stale PUT → 409 `stale_version`, custom line honored, void-issued 409, insufficient stock 409 full rollback, cart re-pricing 2400→2500, gateway pending + payment row + stock untouched, customer cancel voids draft, own-orders/address isolation, empty cart 400, returns 404/400/409 rules), `test/sales.test.ts` + `test/fixtures/pos-worker.ts` (R2 race: two pos-workers on 1 unit → exits [0,3], final stock 0, exactly one order + one issued invoice + one `sale` movement; fixture uses `db.transaction(..., {behavior:"immediate"})` around the real `posCheckout` service, exit 0 = won, 3 = lost 409, 1 = failure). **Docs changed (spec defects, AGENTS.md §4):** (1) schema.md §14 rule 2 — gateway confirmed row needs `gateway`+`gatewayEventId`; the pending checkout row carries only `gatewayPaymentId` and is never updated (webhook inserts a separate confirmed row, deduped on `UNIQUE(gateway, gatewayEventId)`). (2) architecture.md §4.7 — orders header-only; storefront COD pending→confirmed+issue in one tx, gateway stays pending until webhook; customer cancel voids linked draft invoice; payment pending rows stay pending forever (insert-only). (3) architecture.md §4.11 — added outlet resolution rule and gateway-mode contract (pre-gate, cart kept, checkoutReference). (4) api.md §6 — orders header-only, confirm creates empty draft invoice, cancel voids draft, PUT invoice line shapes (client price on regular line → 400, custom at zero tax, duplicate variant → 400), POS body; §9 — products with no visible variant excluded, checkout `checkoutReference`/pending semantics, returns draft rules. |
