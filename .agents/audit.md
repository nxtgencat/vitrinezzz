...
| 2 | `verify-rbac.ts` | Asserts the full RBAC capability matrix from `architecture.md` §4.14: every `(route, capability)` mapping is exercised both directions in a scratch DB through the HTTP layer (`bun test` `test/rbac.test.ts` runs the same matrix against the live app and asserts the middleware + service re-assertion both fire). | A route gaining, losing, or mis-wiring a capability check without the docs and tests moving together. |
| 3 | `verify-stock.ts` | Scratch DB migrated from the live migration set; seeds REF rows directly, then creates batches and writes movements exclusively through the service path (`createBatch`/`writeMovement`, `createdAt` strictly increasing). Independently recomputes `SUM(delta)` per key from `stock_movements` ordered `(createdAt, id)` and asserts the result is byte-identical to the live `stock_levels` — including `lastMovementId`/`updatedAt` — with no negative quantity, no stray rows, and zero rows on an empty DB. | The one projection silently drifting from the fact table it's derived from — a projection bug surfaces as a failed verify, not a silent oversell. |
| 4 | `verify-immutability.ts` | Fires an `UPDATE`/`DELETE` against each `[FACT]` table in a scratch DB and asserts the trigger raises `ABORT`. Also greps for any direct write to a `[FACT]` table across `lib/`, `db/`, `scripts/`, `routes/`, and `index.ts`; writes must live in `services/` (the owning domain services — there is no `lib/ledger.ts`, layout §4.15). Test files are exempt: scenario suites write fact rows as setup by design. | A migration accidentally dropping or weakening a fact-table trigger; a service bypassing the intended write path. |
| 5 | `verify-routes.ts` | Reads every route row in `api.md` §1–§10 and every route mounted on `app.routes`; asserts the two sets are equal in both directions. | A route implemented but undocumented, or documented but never implemented. |
| 6 | `verify-realtime.ts` | Greps for any `publish` call site inside a `withTx`/`withIdempotency` body, and asserts every `‡`-marked route in `api.md` is registered by a route file that calls `publish` (both directions). Publish is a synchronous `server.publish` (§4.8), so this grep — not the compiler — is the backstop keeping publish out of transaction bodies. | A route shipping without its realtime wiring, or a publish call accidentally reachable inside a transaction. |
| 7 | `verify-audit.ts` | Cross-references every mutating route on an audited domain (`architecture.md` §4.12) against its service function's write path, asserting a matching `audit_events` insert exists. | A new catalog/RBAC/settings/media mutation shipping without its required audit row. |
| 8 | `verify-hygiene.ts` | Scans `lib/`, `db/`, `services/`, `routes/`, `scripts/`, `test/`, `index.ts`, `app.ts`, and `drizzle.config.ts` for `console.*`, `any`, and every banned package name (`architecture.md` §2; the scanner's own file is excluded — a linter doesn't flag its own rule definitions); asserts the strict-mode tsconfig flags; runs `bun run typecheck` under `strict: true`; scans route-boundary files (those using `zValidator`/`z.object`) for money fields that are not in the closed client-originable list (`architecture.md` §4.4 — the catalog master prices `costPricePaise`/`sellingPricePaise`/`mrpPaise` joined it in phase 4). | Logging, typing, dependency-boundary, or client-trust discipline eroding silently over time. |
| 9 | `verify-deps.ts` | Scans `lib/`, `db/`, `services/`, `routes/`, `scripts/`, `test/`, `index.ts`, and `app.ts` for every import, resolves each bare specifier to a `package.json` dependency, asserts every declared dependency is imported by at least one file (both directions). Exempt from the orphan direction only: tooling invoked via `bunx` (`typescript`, `@types/bun`, `drizzle-kit`). The deferred list is empty since phase 4 — `zod` and `@hono/zod-validator` are imported by the route boundary; any future deferral needs individual justification at sign-off. | An unused ("orphan") dependency, or an import resolving to nothing declared. |
...
- **R1** — same idempotency key fired twice concurrently → single execution, replay
  byte-identical.
- **R2** — last-unit race, manual-adjustment style: two concurrent consumers on one
  unit of stock (each gating in-tx via `sumStockAtBatch` then writing `adjustment_out`
  −1) → exactly one `200` + one `409 insufficient_stock`, final quantity 0 (never −1,
  never 1). The sales-checkout variant of this race lands with the sales phase.
...
4. Run the replay check against the restored file before resuming traffic, to
   confirm the projection is consistent with the restored fact tables:
   `bun run scripts/verify-stock.ts` does this for the live DB; `lib/stock-check.ts`
   exposes the same projection check (`checkStockProjection(db)`) for any drizzle
   instance — `smoke-ops` exercises it against a restored backup (0 mismatches +
   movement count match), which is the documented restore-time check.
