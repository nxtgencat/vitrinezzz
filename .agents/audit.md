...
| 3 | `verify-immutability.ts` | Fires an `UPDATE`/`DELETE` against each `[FACT]` table in a scratch DB and asserts the trigger raises `ABORT`. Also greps for any direct write to a `[FACT]` table outside `lib/ledger.ts`/the owning domain service. | A migration accidentally dropping or weakening a fact-table trigger; a service bypassing the intended write path. |
...
| 7 | `verify-hygiene.ts` | Greps for `console.*`, `any`, and every banned package name (`architecture.md` §2); runs `bun run typecheck` under `strict: true`; scans for client-supplied money/total fields on mutating routes that shouldn't accept them. | Logging, typing, dependency-boundary, or client-trust discipline eroding silently over time. |
| 8 | `verify-deps.ts` | Scans `lib/`, `db/`, and `scripts/` for every import, resolves each to a `package.json` dependency, asserts every declared dependency is imported by at least one file (both directions). | An unused ("orphan") dependency, or an import resolving to nothing declared. |
