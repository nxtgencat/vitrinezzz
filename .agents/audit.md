...
| 8 | `verify-deps.ts` | Scans `src/` for every import, resolves each to a `package.json` dependency, asserts every declared dependency is imported by at least one file (both directions). | An unused ("orphan") dependency, or an import resolving to nothing declared. |
